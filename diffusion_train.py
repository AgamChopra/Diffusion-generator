...
MODE = int(input(
    'train model(press \'1\') generate synthetic images from previous state(press \'2\'): '))
if MODE == 1:
    load_checkpoint = input(
        'Continue training from previous checkpoint? (yes/no)')
...
class rand_augment():
    def __init__(self):
        self.aug = tf.Compose(
            [tf.RandomRotation(2), tf.ColorJitter(), tf.RandomVerticalFlip()])
...
def apply_random_noise(x, a1, a2, t_encodings):
    ...
    list_len = len(a1)
    idx = torch.tensor(choice(range(0, list_len - 1),
                       x.shape[0]), dtype=torch.long)
    x_t2 = a1[idx].view(len(idx), 1, 1, 1) * x + \
        a2[idx].view(len(idx), 1, 1, 1) * x_epsilon
    t2 = t_encodings[idx]
...
class noise_generator():
    def __init__(self, T=300, time_encoding_dim=128):
        ...
        # getPositionEncoding(T+2,time_encoding_dim)
...
def diff_train(dataset, lr=1E-2, epochs=5, batch=32, T=1000,
               time_encoding_dim=32, load_state=False,
